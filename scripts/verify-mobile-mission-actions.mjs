...
const openPractice = async () => {
  ...
  await evaluate(`document.querySelector(".format-option")?.click()`); await waitFor(".mobile-mission-actions");
};
try {
  await openPractice();
  const visible = await evaluate(`({ change: document.querySelector(".mobile-change-mission")?.textContent?.trim(), end: document.querySelector(".mobile-end-session")?.textContent?.trim() })`);
  ...
  await evaluate(`document.querySelector(".mobile-change-mission")?.click()`); await waitFor(".activity-card");
  await openPractice();
  await evaluate(`document.querySelector(".mobile-end-session")?.click()`); await waitFor(".summary-screen");
  console.log(JSON.stringify({ visible, status: "mobile mission actions valid" }));
