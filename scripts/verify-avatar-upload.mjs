...
try {
  ...
  const hasStoredPhoto = await evaluate(`Boolean(document.querySelector(".profile-avatar-photo"))`);
  if (hasStoredPhoto) {
    await evaluate(`document.querySelector(".profile-avatar-photo > button")?.click()`);
  }
  await waitFor(".profile-avatar-upload input");
  const document = await command("DOM.getDocument", { depth: 1 });
  const node = await command("DOM.querySelector", {
    nodeId: document.root.nodeId,
    selector: ".profile-avatar-upload input",
  });
  ...
  await waitFor(".profile-avatar-photo img.player-avatar-photo");
  const result = await evaluate(`(() => {
    const image = document.querySelector(".profile-avatar-photo img.player-avatar-photo");
  ...
  await sleep(700);
  await waitFor(".profile-avatar-photo img.player-avatar-photo");
  const restored = await evaluate(`({ src: document.querySelector(".profile-avatar-photo img.player-avatar-photo")?.getAttribute("src") ?? "", stored: localStorage.getItem("hana-player-avatar-photo-v1") ?? "", html: document.querySelector(".profile-avatar-photo")?.innerHTML ?? "" })`);
  ...
  await waitFor(".profile-avatar-photo img.player-avatar-photo");
  const selectedAgain = await evaluate(`document.querySelector(".profile-avatar-photo img.player-avatar-photo")?.getAttribute("src") ?? ""`);
