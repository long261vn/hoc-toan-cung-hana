...
export const practiceFormatMeta: Record<
  PracticeFormat,
...
const rewardNames = [
  ...
  "Cúp Phép Tính Hay",
  "Ngôi Sao Siêu Tốc",
  "Huy chương Nhà Phát Minh",
  "La Bàn Sao Bắc Đẩu",
  "Kính Quan Sát Tinh Tường",
  "Hành Tinh Kỳ Diệu",
  "Huy hiệu Bứt Phá",
  "Tàu Thám Hiểm Dũng Mãnh",
  "Cờ Chinh Phục Vàng",
  "Cúp Bầu Trời Sao",
  "Vương Miện Sao Sáng",
  "Huy chương Thiên Tài Nhí",
  "Sao Băng Vinh Quang",
  "Hộp Quà Ngân Hà",
  "Khiên Thuyền Trưởng",
  "Tên Lửa Ước Mơ",
  "Cúp Chinh Phục Tối Cao",
  "Huy hiệu Huyền Thoại Nhí",
  "Hành Tinh Danh Dự",
  "Vương Miện Thiên Hà",
  "Huy hiệu 99 Chòm Sao",
  "Cúp Thuyền Trưởng Hana",
] as const;
...
function rewardDetail(level: number) {
  ...
  if (level === 100)
    return "Bạn đã chinh phục trọn bộ 100 cấp phần thưởng cùng Hana!";
  if (level % 10 === 0)
    return `Bạn đã chạm cột mốc ${level * 10} điểm thật ấn tượng.`;
  return "Bạn đang tiến thêm một bước trên chuyến bay toán học.";
}

export const sessionRewards: SessionReward[] = rewardNames.map(
  (label, index) => {
    const level = index + 1;
    return {
      id: `reward-${level}`,
