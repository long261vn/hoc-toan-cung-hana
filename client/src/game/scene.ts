...
class SpaceMapWorld {
  ...
  private createPlanets() {
    for (const data of planetData) {
      ...
      planet.parent = pivot;
      planet.material = this.material(
        `planet-material-${data.operation}`,
      ...
      crater.material = this.material(
        `crater-material-${data.operation}`,
        data.accent.toHexString(),
        0.22
      );
      ...
      if (data.operation === "multiply") {
        const ring = MeshBuilder.CreateTorus(
          "multiply-ring",
          { diameter: data.radius * 2.7, thickness: 0.12, tessellation: 36 },
          this.scene
        );
        ring.parent = pivot;
        ring.rotation.x = Math.PI * 0.36;
        ring.material = this.material(
          "multiply-ring-material",
          "#FFE798",
          0.28
        );
      }
      ...
      if (data.operation === "divide") {
        const band = MeshBuilder.CreateTorus(
          "divide-band",
          { diameter: data.radius * 2.04, thickness: 0.1, tessellation: 36 },
          this.scene
        );
        band.parent = pivot;
  ...
  private createShip() {
    ...
    const antenna = MeshBuilder.CreateCylinder(
      "ship-antenna",
      { height: 0.58, diameter: 0.08, tessellation: 12 },
      this.scene
    );
    antenna.parent = ship;
  ...
  setActivePlanet(operation: Operation) {
    ...
    this.planets.forEach((pivot, name) => {
      ...
      pivot.position.y =
        planetData.find(planet => planet.operation === name)?.position.y ??
        pivot.position.y;
  ...
  update(delta: number, elapsed: number) {
    this.planets.forEach((pivot, operation) => {
      const base =
        planetData.find(planet => planet.operation === operation)?.position.y ??
        0;
      const phase =
        planetData.findIndex(planet => planet.operation === operation) * 1.2;
      pivot.rotation.y += delta * 0.22;
      pivot.position.y =
        base +
        (operation === this.activeOperation ? 0.18 : 0) +
        Math.sin(elapsed * 0.9 + phase) * 0.05;
    });
  }
...
export async function createGameScene(
  ...
  const coralLight = new PointLight(
    "coralLight",
    new Vector3(-2, 2.5, -3),
    scene
  );
